...
@class NSURL;
@class NSString;
SWIFT_ENUM_FWD_DECL(NSInteger, PostHogDataMode)
@class PostHogRageClickConfig;
@class NSUUID;
@protocol PostHogPropertiesSanitizer;
SWIFT_ENUM_FWD_DECL(NSInteger, PostHogPersonProfiles)
...
@property (nonatomic) NSInteger featureFlagRequestMaxRetries;
...
/// Useful for reverse-proxy setups that require authentication, e.g. an <code>Authorization</code> header.
...
SWIFT_ENUM_FWD_DECL(NSInteger, PostHogDisplaySurveyTextContentType)
...
typedef SWIFT_ENUM(NSInteger, PostHogDisplaySurveyRatingType, open) {
...
typedef SWIFT_ENUM(NSInteger, PostHogDisplaySurveyTextContentType, open) {
...
/// Steps recorded via <code>PostHogSDK.addExceptionStep(_:properties:)</code> accumulate over the run and
/// are attached to every captured <code>$exception</code> as <code>$exception_steps</code>, giving the error tracking
/// UI a timeline of recent activity before each error.
@property (nonatomic, strong) PostHogExceptionStepsConfig * _Nonnull exceptionSteps;
/// List of exception class names (the <code>NSException.name.rawValue</code> /
/// <code>$exception_list[*].type</code> field) that should be skipped during
/// autocapture and manual <code>captureException(_:)</code> instead of being sent
/// as <code>$exception</code> events.
...
/// fatal JS errors via <code>RCTFatal(...)</code> as an <code>NSException</code> named
/// <code>RCTFatalException</code>, which the iOS crash reporter then captures
/// as a separate native crash — duplicating the event the JS layer
/// already captured with its own stack trace. The default <code>["RCTFatalException"]</code>
/// makes the JS-side <code>$exception</code> event the single source of truth out
/// of the box; native iOS apps that never raise that type are
/// unaffected. Mirrors <code>addIgnoredExceptionForType(...)</code> on
/// sentry-react-native’s native side, and pairs with
/// <code>errorTrackingConfig.ignoredExceptionTypes</code> on posthog-android.
...
/// config.errorTrackingConfig.ignoredExceptionTypes = []
///
/// \endcodeSee https://github.com/PostHog/posthog-ios/issues/653.
/// Default: <code>["RCTFatalException"]</code>.
@property (nonatomic, copy) NSArray<NSString *> * _Nonnull ignoredExceptionTypes;
...
/// captured <code>$exception</code>. On a fatal crash they are persisted with the crash context and attached
/// to the crash <code>$exception</code> on the next launch.
SWIFT_CLASS("_TtC7PostHog27PostHogExceptionStepsConfig")
@interface PostHogExceptionStepsConfig : NSObject
/// Whether exception steps are recorded and attached to exceptions.
/// When <code>false</code>, <code>addExceptionStep(_:properties:)</code> is a no-op and no steps are attached.
/// Default: <code>true</code>.
...
SWIFT_ENUM_FWD_DECL(NSInteger, PostHogLogLevel)
...
typedef SWIFT_ENUM(NSInteger, PostHogLogLevel, open) {
...
SWIFT_ENUM_FWD_DECL(NSInteger, PostHogLogSeverity)
@class NSNumber;
...
typedef SWIFT_ENUM(NSInteger, PostHogLogSeverity, open) {
...
///   </li>
/// </ul>
typedef SWIFT_ENUM_NAMED(NSInteger, PostHogPersonProfiles, "PostHogPersonProfiles", open) {
...
/// Each <code>PostHogFeatureFlagResult</code> carries the flag’s <code>key</code>, <code>enabled</code> state,
/// <code>variant</code> (for multivariate flags), and decoded <code>payload</code>. Flags become available
/// after they finish loading from the server; before the first load (or when the SDK
/// is disabled) this returns <code>nil</code>. Reading does not capture <code>$feature_flag_called</code> events.
/// \code
...
/// \endcode
/// returns:
/// An array of <code>PostHogFeatureFlagResult</code>, or <code>nil</code> if no flags are loaded.
- (NSArray<PostHogFeatureFlagResult *> * _Nullable)getAllFeatureFlags SWIFT_WARN_UNUSED_RESULT;
...
/// Steps accumulate in a session-scoped buffer and are attached to <em>every</em> captured <code>$exception</code>
/// as <code>$exception_steps</code>, giving the error tracking UI a timeline of recent activity before each
/// error. The buffer is not cleared by a capture — it rotates only by byte-budget eviction and is
/// cleared on a clean launch or <code>close()</code>. On a fatal crash the buffered steps are persisted with
/// the crash context and attached to the crash <code>$exception</code> reported on the next launch.
/// Reserved keys <code>$message</code> and <code>$timestamp</code> are stripped from <code>properties</code> — the SDK sets the
/// canonical values. The <code>$timestamp</code> is captured at call time so the timeline stays accurate.
...
///
/// \endcode\param message A short, non-empty description of what happened. Empty messages are ignored.
///
/// \param properties Optional additional context to attach to the step.
///
- (void)addExceptionStep:(NSString * _Nonnull)message properties:(NSDictionary<NSString *, id> * _Nullable)properties;
/// Record an exception step without additional properties.
/// Convenience overload for Objective-C callers so <code>properties:</code> doesn’t need to be passed as <code>nil</code>.
/// \param message A short, non-empty description of what happened.
///
- (void)addExceptionStep:(NSString * _Nonnull)message;
- (nonnull instancetype)init SWIFT_UNAVAILABLE;
+ (nonnull instancetype)new SWIFT_UNAVAILABLE_MSG("-init is unavailable");
@end
...
SWIFT_ENUM_FWD_DECL(NSInteger, PostHogSurveyResponseType)
...
typedef SWIFT_ENUM(NSInteger, PostHogSurveyResponseType, open) {
